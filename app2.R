...
server <- function(input, output, session) {
  ...
  process_average_data <- function(filtered_data, variable, demographic, task_key) {
    ...
    # Get ALL demographic values for OVERALL calculations (unfiltered)
    all_demo_values <- c()
    if(length(all_data_averages) > 0 && 
       variable %in% names(all_data_averages) &&
       demographic %in% names(all_data_averages[[variable]])) {
      all_demo_values <- names(all_data_averages[[variable]][[demographic]])
    }
    ...
    # Overall all-data average and SD - USE ALL DEMOGRAPHIC VALUES (unfiltered)
    overall_all_data_count <- 0
    overall_all_data_sum <- 0
    overall_all_data_avg <- NA
    overall_all_data_sd <- NA
    
    # Use pre-calculated averages to compute overall statistics using ALL data
    ...
    # Debug output
    if(!is.na(overall_all_data_avg)) {
      cat("Overall All Data - Count:", overall_all_data_count, "Avg:", overall_all_data_avg, "\n")
    }
    
    # For overall SD, use ALL demographic values (unfiltered)
